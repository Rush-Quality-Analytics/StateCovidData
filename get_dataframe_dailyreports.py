...
lofl = []
for i, ps in enumerate(provstates):
...
df_main['states_admin2'] = df_main['Province/State'] + '-' + df_main['Admin2']
states_admin2 = sorted(list(set(df_main['states_admin2'].tolist())))

print(len(states_admin2), 'counties, etc.')

for val in states_admin2:
    
    tdf = df_main[df_main['states_admin2'] == val]
    tdf.drop(['states_admin2'], axis=1, inplace=True)
    tdf.to_csv(mydir + 'data/' + val + '-COVID-CASES.txt', sep='\t')
